...
pipelineJob("${JOB_FOLDER}/${JOB_NAME}") {
    ...
    parameters {
        ...
        stringParam {
            ...
            defaultValue('')
            description('Gerrit SSH user; leave empty if you are using anonymous HTTPS access')
        }
        stringParam {
            name ('GERRIT_SSH_CREDS')
            defaultValue('')
            description('Gerrit SSH credentials stored on Jenkins; leave empty if you are using anonymous HTTPS access')
        }
