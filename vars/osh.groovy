 ...
 *
 * @param map Openstack component images
 * @param mos false - default - get community image versions
              true - get mos image versions
 */
def imageOverrides(Map map, Boolean mos = false) {

    // safe way to check for ps image, or set to default
    KEYSTONE_LOCI = map.find{ it.key == "KEYSTONE_LOCI" }?.value ?: (mos ? ncconf.MOS_KEYSTONE_LOCI : ncconf.KEYSTONE_LOCI)
    HEAT_LOCI = map.find{ it.key == "HEAT_LOCI" }?.value ?: (mos ? ncconf.MOS_HEAT_LOCI : ncconf.HEAT_LOCI)
    HORIZON_LOCI = map.find{ it.key == "HORIZON_LOCI" }?.value ?: (mos ? ncconf.MOS_HORIZON_LOCI : ncconf.HORIZON_LOCI)
    GLANCE_LOCI = map.find{ it.key == "GLANCE_LOCI" }?.value ?: (mos ? ncconf.MOS_GLANCE_LOCI : ncconf.GLANCE_LOCI)
    CINDER_LOCI = map.find{ it.key == "CINDER_LOCI" }?.value ?: (mos ? ncconf.MOS_CINDER_LOCI : ncconf.CINDER_LOCI)
    NOVA_LOCI = map.find{ it.key == "NOVA_LOCI" }?.value ?: (mos ? ncconf.MOS_NOVA_LOCI : ncconf.NOVA_LOCI)
    NOVA_1804_LOCI = map.find{ it.key == "NOVA_1804_LOCI" }?.value ?: (mos ? ncconf.MOS_NOVA_1804_LOCI : ncconf.NOVA_1804_LOCI)
    NEUTRON_LOCI = map.find{ it.key == "NEUTRON_LOCI" }?.value ?: (mos ? ncconf.MOS_NEUTRON_LOCI : ncconf.NEUTRON_LOCI)
    NEUTRON_SRIOV_LOCI = map.find{ it.key == "NEUTRON_SRIOV_LOCI" }?.value ?: (mos ? ncconf.MOS_NEUTRON_SRIOV_LOCI : ncconf.NEUTRON_SRIOV_LOCI)
 ...
 * @param mos false - default - get community image versions
              true - get mos image versions
 */
def serviceVersions(Map map, Boolean mos = false) {
    sh 'mkdir -p $WORKSPACE/artifacts'
