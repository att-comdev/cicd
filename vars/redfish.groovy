...
def getPowerState (String ip, String auth, systemPath=null) {

    systemPath = systemPath ?: getSystemPath(ip, auth, systemPath)
...
def setPowerState (String ip, String auth, String state, systemPath=null) {
...
def powerOn (String ip, String auth, systemPath=null) {

    if (getPowerState(ip, auth, systemPath) == 'On') {
        print "Power state already On, skipping"
        return 0
    }

    print "Setting power state On for node ${ip}"
    setPowerState(ip, auth, 'On', systemPath)

    timeout(1) {
        def state = getPowerState(ip, auth, systemPath)
        while (state != 'On') {
            print "Power state is not yet \'On\' (${state}), waiting 10 seconds"
            sleep 10
            state = getPowerState(ip, auth, systemPath)
        }
    }
}

def powerOff (String ip, String auth, systemPath=null) {

    if (getPowerState(ip, auth, systemPath) == 'Off') {
        print "Power state already Off, skipping"
        return 0
    }

    print "Setting power state ForceOff for node ${ip}"
    setPowerState(ip, auth, 'ForceOff', systemPath)
...
def powerReset (String ip, String auth, systemPath=null) {
    ...
    powerOff(ip, auth, systemPath)
    powerOn(ip, auth, systemPath)
}
