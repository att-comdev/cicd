...
//  - JENKINS_URL
//  - JENKINS_CLI
def credentialsId='jenkins-openstack-18'
...
def stack_create(String name, String tmpl, String parameters) {
  ...
  withCredentials([usernamePassword(credentialsId: '${credentialsId}',
        ...
        cmd = openstack_cmd("openstack stack create --wait --timeout 15 -t /target/\$(basename ${tmpl}) ${name} 'cloudImage=${parameters}'", "\$(dirname ${tmpl})")
...
def stack_delete(String name) {
    withCredentials([usernamePassword(credentialsId: '${credentialsId}',
      ...
      cmd = openstack_cmd("openstack stack delete --wait --timeout 15 --yes ${name}")
        code = sh (script: cmd, returnStatus: true)
        if (!code) {
