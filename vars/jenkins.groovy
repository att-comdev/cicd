...
def node_create(String name, String host, String port = '22',
    ...
    withCredentials([usernamePassword(credentialsId: 'jenkins-token',
        ...
        cmd = "keytool -import -alias JENKINS-CONTROLLER-CERT  -keystore $JAVA_HOME/jre/lib/security/cacerts -file /var/jenkins_home/ca.crt -storepass changeit -noprompt;echo '${config}' | java -jar \$JENKINS_CLI ${opts} create-node ${name}"
...
def node_delete(String name) {
    withCredentials([usernamePassword(credentialsId: 'jenkins-token',
        ...
        cmd = "keytool -import -alias JENKINS-CONTROLLER-CERT  -keystore $JAVA_HOME/jre/lib/security/cacerts -file /var/jenkins_home/ca.crt -storepass changeit -noprompt;java -jar \$JENKINS_CLI $opts delete-node $name"
