...
def call(name, tmpl, Closure body) {
    ...
    try {
        ...
        // body executed under specified slave node
        node (name) {
            body()
        }
        ...
        slack.slack_msg("Pipeline failed: ${error}")
        error(error)

    } finally {
        node(launch_node) {
            jenkins_slave_destroy(name)
        }
    }
}
