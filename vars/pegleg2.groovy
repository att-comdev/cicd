...
def encrypt(siteRepo, username, sshKey, author, siteName, peglegPassphrase, peglegSalt) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        -e PEGLEG_PASSPHRASE=${peglegPassphrase} -e PEGLEG_SALT=${peglegSalt} $conf.PEGLEG_IMAGE -e pegleg.sh -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets encrypt -a ${author} ${siteName}"
}
