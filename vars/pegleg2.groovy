...
def generatePkiWithinContainer(siteRepo, globalRepo, username, sshKey, author, siteName) {
    sh "pegleg -v site -r ${siteRepo} -e global=${globalRepo} -u ${username} -k ${sshKey} secrets generate-pki -a ${author} ${siteName}"
}
...
def generatePki(siteRepo, username, sshKey, author, siteName) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        $conf.PEGLEG_IMAGE pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets generate-pki -a ${author} ${siteName}"
}
...
def generatePkiWithinContainer(siteRepo, username, sshKey, author, siteName) {
    sh "pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets generate-pki -a ${author} ${siteName}"
}