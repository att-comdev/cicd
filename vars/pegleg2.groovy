...
def encrypt(siteRepo, globalRepo, username, ssh_key, author, projectSite) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        $conf.PEGLEG_IMAGE pegleg.sh -v site -r ${siteRepo} -e global=${globalRepo} -u ${username} -k ${ssh_key} secrets encrypt -a ${author} ${projectSite}"
}
...
def encryptWithinContainer(siteRepo, globalRepo, username, ssh_key, author, projectSite) {
    sh "pegleg.sh -v site -r ${siteRepo} -e global=${globalRepo} -u ${username} -k ${ssh_key} secrets encrypt -a ${author} ${projectSite}"
}