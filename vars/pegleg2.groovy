 ...
 * @param siteName The name of the site you're looking to render. Must match what's in your site repository's site-definition.yaml
 */
def generatePki(siteRepo, username, sshKey, author, siteName, peglegPassphrase, peglegSalt) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        -e PEGLEG_PASSPHRASE=${peglegPassphrase} -e PEGLEG_SALT=${peglegSalt} $conf.PEGLEG_IMAGE pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets generate-pki -a ${author} ${siteName}"
}
...
def generatePkiWithinContainer(siteRepo, username, sshKey, author, siteName, peglegPassphrase, peglegSalt) {
    sh """PEGLEG_PASSPHRASE="${peglegPassphrase}"; PEGLEG_SALT="${peglegSalt}"; pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets generate-pki -a ${author} ${siteName}"""
}