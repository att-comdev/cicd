 ...
 */
def generateGenesis(siteRepo, username, sshKey, destinationDirectory, encryptionKey, siteName) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        -e PEGLEG_PASSPHRASE=${peglegPassphrase} -e PEGLEG_SALT=${peglegSalt} $conf.PEGLEG_IMAGE pegleg -v site -u ${username} -k ${sshKey} -r ${siteRepo} genesis_bundle -b ${destinationDirectory} -k ${encryptionKey} ${siteName}"
}
 ...
 * @param siteName The name of the site you're looking to generate scripts for. Must match what's in your site repository's site-definition.yaml
 */
def generateGenesisWithinContainer(siteRepo, username, sshKey, destinationDirectory, encryptionKey, siteName, peglegPassphrase, peglegSalt) {
    sh """export PEGLEG_PASSPHRASE="${peglegPassphrase}"; export PEGLEG_SALT="${peglegSalt}"; pegleg -v site -u ${username} -k ${sshKey} -r ${siteRepo} genesis_bundle -b ${destinationDirectory} -k ${encryptionKey} ${siteName}"""
}