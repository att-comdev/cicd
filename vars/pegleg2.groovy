...
def dockerExecRender(siteRepo, username, sshKey, siteName) {
  sh """sudo docker exec -i pegleg pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} collect ${siteName} -o ${siteName}.yaml"""
}

def dockerExecEncrypt(peglegPassphrase, peglegSalt, siteRepo, username, sshKey, siteName) {
  sh """sudo docker exec -i pegleg /bin/bash -c "export PEGLEG_PASSPHRASE="${peglegPassphrase}" && export PEGLEG_SALT="${peglegSalt}"; pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets encrypt -a ${username} ${siteName}"""
}

def dockerExecDecrypt(peglegPassphrase, peglegSalt, siteRepo, username, sshKey, siteName, securityRepo) {
  sh """sudo docker exec -i pegleg /bin/bash -c "export PEGLEG_PASSPHRASE="${peglegPassphrase}" && export PEGLEG_SALT="${peglegSalt}"; pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets decrypt -a ${username} -f ${securityRepo}"""
}