...
def generateGenesisWithinContainer(siteRepo, username, sshKey, destinationDirectory, siteName, peglegPassphrase, peglegSalt) {
    sh """export PEGLEG_PASSPHRASE="${peglegPassphrase}"; export PEGLEG_SALT="${peglegSalt}"; pegleg -v site -u ${username} -k ${sshKey} -r ${siteRepo} genesis_bundle -b ${destinationDirectory} ${siteName}"""
}