...
def lintWithinContainer(siteRepo, globalRepo, secretsRepo, username, sshKey, siteName, type) {
  if(type == "directory"){
      ...
      sh "pegleg -v site -r ${siteRepo} -e global=${globalRepo} -e secrets=${secretsRepo} -u ${username} -k ${sshKey} lint ${projectSite} -x P001 -x P003"
  }
}
...
def lintWithinContainer(siteRepo, username, sshKey, siteName, type) {
    sh "pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} lint ${projectSite} -x P001 -x P003"
}
...
def render(siteRepo, globalRepo, username, sshKey, siteName) {
    sh "sudo docker run --rm -i --net=none --workdir='/workspace' -v \$(pwd):/workspace \
        $conf.PEGLEG_IMAGE pegleg site -p ${siteRepo} -e global=${globalRepo} -u ${username} -k ${sshKey} render ${projectSite} -o ${siteName}.yaml"
}
...
def renderWithinContainer(siteRepo, globalRepo, secretsRepo, username, sshKey, siteName, type) {
  if (type == "directory"){
    ...
    sh "pegleg site -p ${siteRepo} -e global=${globalRepo} -u ${username} -k ${sshKey} render ${projectSite} -o ${siteName}.yaml"
  }
}
...
def renderWithinContainer(siteRepo, username, sshKey, siteName, type) {
    sh "pegleg site -p ${siteRepo} -u ${username} -k ${sshKey} render ${projectSite} -o ${siteName}.yaml"
}
...
def collect(siteRepo, globalRepo, username, sshKey, projectSite) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        $conf.PEGLEG_IMAGE pegleg site -p ${siteRepo} -e global=${globalRepo} -u ${username} -k ${sshKey} collect ${projectSite} -s ${siteName}"
}
