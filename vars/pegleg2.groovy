...
def encrypt(siteRepo, username, sshKey, author, siteName, peglegPassphrase, peglegSalt) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        $conf.PEGLEG_IMAGE pegleg.sh -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets encrypt -a ${author} ${siteName} PEGLEG_PASSPHRASE=${peglegPassphrase} PEGLEG_SALT=${peglegSalt}"
}
...
def encryptWithinContainer(siteRepo, username, sshKey, author, siteName, passphrase, salt) {
    sh "pegleg.sh -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets encrypt -a ${author} ${siteName} PEGLEG_PASSPHRASE=${peglegPassphrase} PEGLEG_SALT=${peglegSalt}"
}
