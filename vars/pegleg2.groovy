...
def encrypt(siteRepo, username, sshKey, author, siteName, peglegPassphrase, peglegSalt) {
    sh "docker run --rm -i --net=none --workdir=/workspace -v \$(pwd):/workspace \
        -e PEGLEG_PASSPHRASE=${peglegPassphrase} -e PEGLEG_SALT=${peglegSalt} $conf.PEGLEG_IMAGE pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets encrypt -a ${author} ${siteName}"
}
...
def encryptWithinContainer(siteRepo, username, sshKey, author, siteName, passphrase, salt) {
    sh """PEGLEG_PASSPHRASE="${peglegPassphrase}"; PEGLEG_SALT="${peglegSalt}"; pegleg -v site -r ${siteRepo} -u ${username} -k ${sshKey} secrets encrypt -a ${author} ${siteName}"""
}
