 ...
 */
def getSteps(action, shipyardUrl, keystoneCredId, keystoneUrl, withCreds) {
 ...
 */
def getState(systep, shipyardUrl, keystoneCredId, keystoneUrl, withCreds) {
    def req = keystone.retrieveToken(keystoneCredId, keystoneUrl, withCreds)
    def token = req.getHeaders()["X-Subject-Token"][0]
    def res = null
    retry(3) {
 ...
 * @param withCreds Boolean. Flag for using jenkins configuration to get keystone credentials.
 */
def waitStep(systep, interval, shipyardUrl, keystoneCredId, keystoneUrl, withCreds) {
 ...
 * @param withCreds Boolean. Flag for using jenkins configuration to get keystone credentials.
 */
def waitSteps(action, shipyardUrl, keystoneCredId, keystoneUrl, withCreds) {
