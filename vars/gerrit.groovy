...
def cloneRepository(String url, String refspec, Boolean shallow = null, String creds = null, String gitCmd = null, String targetDirectory = null, String localBranch = null) {
    shallow = shallow ?: checkShallowVariable()
    ...
    def runGit = {
        sh """
            ...
            git fetch ${shallow ? "--depth=${env.SHALLOW_DEPTH ?: 2}" : ''} ${env.GIT_OPTIONS ?: '-q'} origin ${refspec} && git checkout FETCH_HEAD ${localBranch ? "-b \"$localBranch\"" : ''}
        """
    }
...
def clone(String url, String refspec) {
    cloneRepository(url, GERRIT_REFSPEC)
}

@Deprecated
def clone(String url, String refspec, String creds) {
    cloneRepository(url, GERRIT_REFSPEC, creds = creds)
}

@Deprecated
def _cloneShallowCmd(url, branch, gitSshCommand='') {
    cloneRepository(url, branch, gitCmd = gitSshCommand, shallow = true)
}

@Deprecated
def _cloneShallow(url, branch, targetDirectory, gitSshCommand='') {
    cloneRepository(url, branch, gitCmd = gitSshCommand, shallow = true, targetDirectory = targetDirectory)
}

@Deprecated
def _clone(url, branch, targetDirectory, refspec=null) {
    cloneRepository(url, refspec ?: GERRIT_REFSPEC, targetDirectory = targetDirectory, localBranch = 'jenkins')
}

@Deprecated
def _cloneWithCredsShallow(url, branch, targetDirectory, creds, refspec) {
    cloneRepository(url, refspec, shallow = true, targetDirectory = targetDirectory, creds = creds)
}

@Deprecated
def _cloneWithCreds(url, branch, targetDirectory, creds, refspec) {
    cloneRepository(url, refspec, targetDirectory = targetDirectory, creds = creds, localBranch = 'jenkins')
}

@Deprecated
def cloneToBranch(String url, String refspec, String targetDirectory) {
    cloneRepository(url, refspec, targetDirectory = targetDirectory)
}
...
def cloneToBranch(String url, String refspec, String targetDirectory, String creds, String gerritRefspec) {
    cloneRepository(url, gerritRefspec, targetDirectory = targetDirectory, creds = creds)
}
...
def cloneToBranch(String url, String refspec, String targetDirectory, String creds) {
    cloneRepository(url, GERRIT_REFSPEC, targetDirectory = targetDirectory, creds = creds)
}
...
def cloneProject(String url, String branch, String refspec, String targetDirectory) {
    ...
    if (shallowEnabled) {
        cloneRepository(url, branch, targetDirectory = targetDirectory, shallow = true)
    } else {
        cloneRepository(url, refspec, targetDirectory = targetDirectory, shallow = false)
    }
}
...
def cloneProject(String url, String branch, String refspec, String targetDirectory, String creds) {
    cloneRepository(url, refspec, targetDirectory = targetDirectory, creds = creds)
}
