...
def _setupKnownHosts() {
    ...
    if (env.KNOWN_HOSTS) {
        wrap([$class: 'MaskPasswordsBuildWrapper', varPasswordPairs: [[password: KNOWN_HOSTS, var: 'data']]]) {
            cmd = "echo \"${KNOWN_HOSTS}\" >> ${homeSshDir}/known_hosts"
            res = sh(returnStdout: true, script: cmd)
        }
    }
    echo "Known hosts file ${homeSshDir} was updated."
    return homeSshDir
