 ...
 * @param creds jenkins SSH credentials ID
*/
def cloneToRelativeDir(String url, String refspec, String targetDirectory){
    checkout poll: false,
    scm: [$class: 'GitSCM',
              branches: [[name: refspec]],
              doGenerateSubmoduleConfigurations: false,
...
/**
 * Clones Git repository via SSH to the target directory
 *
 * @param url "ssh://${GERRIT_HOST}/${GERRIT_PROJECT}" string
 * @param refspec "xxxx/master" or other refspec
 * @param targetDirectory local directory where to clone repo
 * @param creds jenkins SSH credentials ID
*/
def cloneToRelativeDir(String url, String refspec, String targetDirectory, String creds){
    checkout poll: false,
    scm: [$class: 'GitSCM',
              branches: [[name: refspec]],
              doGenerateSubmoduleConfigurations: false,
...
def getTopicCommitId(repo, url, port) {
    ...
    if("${GERRIT_TOPIC}" != null && "${GERRIT_TOPIC}" != "") {
        def topicJson = sh(script: "ssh -p ${gerritPort} ${gerritUrl} gerrit query --format=JSON topic:${GERRIT_TOPIC} status:open project:${repo}", returnStdout: true).trim()
        def topicData = new JsonSlurperClassic().parseText(topicJson)
        def changeId = topicData.id
        if(changeId != null && changeId != "") {
            def commitJson = sh(script: "ssh -p ${gerritPort} ${gerritUrl} gerrit query --format=JSON --current-patch-set ${changeId}", returnStdout: true).trim()
