...
def cloneRepository(args) {
    ...
    // also, the '.git' part is removed
    // this is consistent with 'git clone' command behavior.
    args.targetDirectory = args.targetDirectory ?: new URI(args.url).path.split('/').last().trim().replaceFirst(/(?i)\.git$/,'')

    // make folder for cloned repo (and display warning if it exists)
    sh "mkdir \"${args.targetDirectory}\" || echo '[WARNING] Unable to create target directory: ${args.targetDirectory}'"
...
/**
 * Clones upstream git repository
 *
 * @param repo - name of the repository
 * @param refspec - refspec to clone
 * @param shallow - perform shallow clone or not
 ...
 * @param gitCmd - override default GIT_SSH_COMMAND variable
 */
def cloneUpstream(args) {
    if (!args.repo || !args.refspec) {
        throw new IllegalArgumentException("Gerrit cloneDownstream: 'repo' and 'refspec' parameters are mandatory")
    }

    cloneRepository url: "https://review.gerrithub.io/${args.repo}",
                    refspec: args.refspec,
                    shallow: args.shallow,
                    creds: args.creds,
                    targetDirectory: args.targetDirectory,
...
def submitPatchset(credentials, userEmail, userName, commitMessage, gerritUrl, repoName, refspec = "refs/for/master", workDir=null) {
    ...
    withCredentials([sshUserPrivateKey(credentialsId: credentials,
        keyFileVariable: 'SSH_KEY')]) {
        dir(workDir) {
            runScp("${sshParams} -p -O -P 29418 ${gerritUrl}:hooks/commit-msg .git/hooks")
            sh """
...
//Please use cloneRepository/cloneUpstream/cloneDownstream - they can replace any of the methods below.
...
def clone(String url, String refspec) {
    ...
    cloneRepository(url: url, refspec: refspec, targetDirectory: '.', shallow: false)
}
...
def clone(String url, String refspec, String creds) {
    ...
    cloneRepository(url: url, refspec: refspec, creds: creds, targetDirectory: '.', shallow: false)
}
