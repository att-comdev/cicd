 ...
 * @param refspec "xxxx/master" or other refspec
 */
def submitPatchset(credentials, userEmail, userName, commitMessage, gerritUrl, repoName, refspec = "refs/for/master", gitDir=null) {
    gitDir = gitDir != null ? gitDir: repoName
    knownHostsFile = _setupKnownHosts()
 ...
 * @param refspec "xxxx/master" or other refspec
 */
def submitPatchsetWithTopic(credentials, userEmail, userName, commitMessage, gerritUrl, repoName, refspec = "refs/for/master", gerritTopic = "", gitDir=null) {
    gitDir = gitDir != null ? gitDir: repoName
    knownHostsFile = _setupKnownHosts()
    sshParams = "-i \${SSH_KEY} -o UserKnownHostsFile=${knownHostsFile} -o StrictHostKeyChecking=yes"
    withCredentials([sshUserPrivateKey(credentialsId: credentials,
 ...
 * @param refspec "xxxx/master" or other refspec
 */
def amendPatchset(credentials, userEmail, userName, gerritUrl, repoName, refspec = "refs/for/master", gitDir=null) {
    gitDir = gitDir != null ? gitDir: repoName
    knownHostsFile = _setupKnownHosts()
    withCredentials([sshUserPrivateKey(credentialsId: credentials,
        keyFileVariable: 'SSH_KEY')]) {
 ...
 * @param refspec "xxxx/master" or other refspec
 */
def amendPatchsetWithTopic(credentials, userEmail, userName, gerritUrl, repoName, refspec = "refs/for/master", gerritTopic = "", gitDir=null) {
    gitDir = gitDir != null ? gitDir: repoName
    knownHostsFile = _setupKnownHosts()
    withCredentials([sshUserPrivateKey(credentialsId: credentials,
        keyFileVariable: 'SSH_KEY')]) {
