...
/**
 * Helper method for invoking and returning _getAction.
 *
 *  @param map The map containing our request parameters.
 *  @param actionId The Shipyard action's ID.
 */
def _getActionResponse(Map map, actionId) {
    return _getAction(actionId: actionId,
    ...
    while (status == "Pending" || status == "Processing") {
        ...
        if (map.genesisCreds && map.genesisIp && printStatus) {
            if ('drydock_build' in failedSteps || 'drydock_build' in runningSteps) {
                ...
                ssh.wait (map.genesisCreds, map.genesisIp,
                          "sudo kubectl --kubeconfig=/etc/kubernetes/admin/kubeconfig.yaml get deployments --all-namespaces | awk '\$4 != \$5' | grep -vE '([0-9]+)/\\1'")
