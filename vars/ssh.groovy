...
def getRemote (Map map) {
    ...
    withCredentials([sshUserPrivateKey(credentialsId: map.sshDetails.creds,
        keyFileVariable: 'SSH_KEY',
        usernameVariable: 'SSH_USER')]) {

        def remote = [:]
        remote.name = 'remote'
        remote.host = map.sshDetails.ip
        remote.allowAnyHosts = true
        ...
        if (map.sshDetails.containsKey('proxy') && map.sshDetails.proxy != null && map.sshDetails.proxy.size() > 0) {
            def proxy = [:]
            proxy.name = 'proxy'
            proxy.host = map.sshDetails.proxy.proxyHost
            proxy.port = map.sshDetails.proxy.proxyPort
            proxy.type = 'HTTP'
            remote.proxy = proxy
        }
