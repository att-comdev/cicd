...
copy_seed(){
    ...
    for seed in $seed_list; do
        ...
        if [ -f ${seed_file} ]; then
            ...
            cp -a ${seed_file} ${WORKSPACE}/${BUILD_NUMBER}/${seed_dir}_seed.groovy
        else
...
lint_jenkins_files(){
    ...
    for file in ${MODIFIED_FILES}; do
        ...
        #this requires anonymous read access
        #cat "${file}" | java -jar /var/jenkins_home/war/WEB-INF/jenkins-cli.jar declarative-linter
        #this requires ssh connection on a fixed port
        #ssh <user>@localhost -p <port> declarative-linter < $file
    done
}
...
lint_whitespaces
#temporarily disable linter check until ssh access or anonymous read access is enabled
#lint_jenkins_files
find_seed
set +x
