    ...
    stage('Seed jobs') {
        ...
        if (!changedJobSeeds.any()) {
            ...
            changedJobSeeds.each {
                try {
                    def text = correctDependencyPaths(it)
                    println "[Info] Seeding \"${it}\""
                    println env.getEnvironment()
                    println text
                    dir(projectFolder.toString()) {
                        jobDsl scriptText: text,
                            removedJobAction: 'IGNORE',
                            removedViewAction: 'IGNORE',
                            removedConfigFilesAction: 'IGNORE',
                            additionalParameters: env.getEnvironment()
                    }
                } catch (Exception e) {
                    println "error: ${e}"
...
def findSeedGroovy(file) {
    ...
    if (fileMatcher.matches()) {
        ...
        foundSeeds = new FileNameFinder().getFileNames(directory, "*.seed.groovy").collect { new File(it) }
        if (foundSeeds.size()) {
            return foundSeeds
        }
...
def correctDependencyPaths(file) {
    ...
    if (dependencyMatcher.any()) {
        ...
        for (dependencyExpression in dependencyMatcher) {
            ...
            def resolvedFile = new File(env.getEnvironment().expand(dependencyFilePath))
            if (resolvedFile.exists()) {
                ...
                if (filesWithMatchingName.size() != 1) {
                    ...
                    println dependencyExpression.toString()
                    println dependencyFilePath
                    println filesWithMatchingName.first()
                    println dependencyExpression.toString().replace(dependencyFilePath, filesWithMatchingName.first())
