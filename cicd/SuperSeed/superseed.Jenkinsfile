...
def correctDependencyPaths(file) {
    ...
    dependencyMatcher = file.text =~ /evaluate\s*\(?\s*[\"\'](.*?\.groovy)[\"\']\s*\)?/
        ...
        for (dependencyExpression in dependencyMatcher) {
            ...
            if (resolvedFile.exists) {
                ...
                if (filesWithMatchingName.size() != 1) {
                    ...
                    file.text = file.text.replace(dependencyExpression.toString(), dependencyExpression.toString().replace(dependencyFilePath, filesWithMatchingName.first()))
                }
            }
        }
    }
