...
node('controller') {
    ...
    stage('Clone git repository') {
        ...
        if (GERRIT_HOST.contains('review')) {
            gerrit.cloneToBranch("https://review.gerrithub.io/$GERRIT_PROJECT", GERRIT_REFSPEC, GERRIT_PROJECT)
        } else {
            gerrit.cloneToBranch("$INTERNAL_GERRIT_SSH/$GERRIT_PROJECT", GERRIT_REFSPEC, GERRIT_PROJECT, INTERNAL_GERRIT_KEY)
        }
        projectFolder = new File(WORKSPACE, GERRIT_PROJECT)
    }
