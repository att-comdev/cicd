        ...
        if (!changedJobSeeds.any()) {
            ...
            changedJobSeeds.each {
                try {
                    ...
                    jobDsl scriptText: (new File(WORKSPACE, it).text),
            ...
            if (errors.size()) {
                ...
                summaryText += errors.map { "<li><b>${it[0]}:</b> <span style='color:#AA0000'>${it[1]}</span></li>" }.join()
                summaryText += '</ul>'
...
def findSeedGroovy(file) {
    ...
    if (fileMatcher.matches()) {
        ...
        if (baseFileName) {
            foundSeeds = findFiles(new File(WORKSPACE, directory), "${java.util.regex.Pattern.quote(baseFileName)}\\.seed\\.groovy")
            if (foundSeeds.size()) {
        ...
        foundSeeds = findFiles(new File(WORKSPACE, directory), '.*seed.*\\.groovy')
        if (foundSeeds.size()) {
            return foundSeeds.collect { new File(directory, it).toString() }
        }
