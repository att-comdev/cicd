      ...
      container('ubuntu-bionic') {
        ...
        stage("Clone Airshipctl Repo") {
          writeFile file: 'cloneRepo.sh', text: '''#!/bin/bash
            ...
            sed -i 's/<project_id>/'''+ env.PROJECT_ID +'''/g' manifests/site/gcp-test-site/target/controlplane/project_name.json
            sed -i '/value:.*/s//value: '''+ env.TARGET_REGION +'''/g' manifests/site/gcp-test-site/target/controlplane/region_name.json
            sed -i '/value:.*/s//value: '''+ env.CONTROLPLANE_VM_TYPE +'''/g' manifests/site/gcp-test-site/target/controlplane/machine_type.json
            sed -i '/value:.*/s//value: '''+ env.CONTROLPLANE_COUNT +'''/g' manifests/site/gcp-test-site/target/controlplane/machine_count.json
            sed -i '/value:.*/s//value: '''+ env.WORKER_VM_TYPE +'''/g' manifests/site/gcp-test-site/target/workers/machine_type.json
            sed -i '/value:.*/s//value: '''+ env.WORKER_COUNT +'''/g' manifests/site/gcp-test-site/target/workers/machine_count.json
            '''
        ...
        withCredentials([azureServicePrincipal('AZURE_CLOUD_ERIC')]) {
          stage("Setup Env Vars") {
            ...
            writeFile file: 'site.sh', text: '''
               export TEST_SITE="gcp-test-site"
