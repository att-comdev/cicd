...
node(targetVM) {
	withEnv(['AIRSHIP_SRC=/tmp/airship',
		'KUBECONFIG=/home/capo/.airship/kubeconfig',
		'WORKERS_COUNT=2',
	...
	try{
		...
		stage("Clusterctl Move"){
			...
			cd $AIRSHIPCTL && ./tools/deployment/provider_common/33_cluster_move_target_node.sh'''
			sh 'bash ./cluster-move.sh'
		}
		stage("Deploy Worker Node"){
			writeFile file: 'worker-node.sh', text: '''#!/bin/bash
			cd $AIRSHIPCTL && WORKERS_COUNT="2" KUBECONFIG="/tmp/target-cluster.kubeconfig" SITE="openstack-test-site" ./tools/deployment/provider_common/34_deploy_worker_node.sh'''
			sh 'bash ./worker-node.sh'
		}
		stage("Cleanup Openstack Target CAPI Resources"){
			writeFile file: 'cleanup_openstack_resources.sh', text: '''#!/bin/bash
			cd $AIRSHIPCTL && ./tools/deployment/provider_common/35_cleanup_capi_resources.sh'''
			sh 'bash ./cleanup_openstack_resources.sh'
		}
	}catch (err) {
		echo "Failed: ${err}"
