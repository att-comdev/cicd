...
def chartsJson = '''{ "airship":[{
                        "repo_name":"airship-promenade",
                        "git_repo":"openstack/airship-promenade",
                        "ref":"master"
                    }]}'''
...
for (entry in object.airship) {
    pipelineJob("charts/airship/${entry.repo_name}") {
        ...
        parameters {
            stringParam('PROMENADE_PROJECT',"${entry.git_repo}")
        ...
        triggers {
            ...
            definition {
                cps {
                    script(readFileFromWorkspace('charts/airship/airship-promenade/Jenkinsfile'))
                    sandbox(false)
                }
            }
        }
