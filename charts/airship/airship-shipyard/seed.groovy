...
def chartsJson = '''{ "airship":[{
                        "repo_name":"airship-shipyard",
                        "git_repo":"openstack/airship-shipyard",
                        "ref":"master"
                    }]}'''
...
for (entry in object.airship) {
    pipelineJob("charts/airship/${entry.repo_name}") {
        ...
        parameters {
            stringParam('SHIPYARD_PROJECT',"${entry.git_repo}")
        ...
        triggers {
            ...
            definition {
                cps {
                    script(readFileFromWorkspace('charts/airship/airship-shipyard/Jenkinsfile'))
                    sandbox(false)
                }
            }
        }
