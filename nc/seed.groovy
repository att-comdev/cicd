...
pipelineJob("${JOB_NAME}") {
    parameters {
        ...
        stringParam {
            name ('AIRSHIPCTL_REF')
            defaultValue('892bb6a16d53a0f43571284db83901bef53ed440')
...
-----END PGP PUBLIC KEY BLOCK-----''')
    ...
    definition {
        cpsScm {
          scm {
	    git([url: 'https://review.gerrithub.io/att-comdev/cicd', branch: 'refs/changes/09/515109/6'])
	  }
...
JOB_FOLDER="nc/Treasuremap"
JOB_NAME="Treasuremapv2-AirshipctlMaster"
pipelineJob("${JOB_NAME}") {
    parameters {
        stringParam {
            name ('GERRIT_REFSPEC')
...
-----END PGP PUBLIC KEY BLOCK-----''')
    ...
    definition {
        cps {
          script(readFileFromWorkspace("${JOB_FOLDER}/Treasuremapv2-AirshipctlMaster.groovy"))
            sandbox(true)
        }
    }
}
