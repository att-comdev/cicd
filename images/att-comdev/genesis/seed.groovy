...
pipelineJob("${JOB_FOLDER}/${JOB_NAME}") {
  ...
  parameters {
    ...
    stringParam {
      ...
      description('IP address of Genesis out-of-band management interface (only iDRAC is tested)')
    }
    stringParam {
    ...
    stringParam {
      ...
      description('IP address mask of Genesis server (x.x.x.x)')
    }
    stringParam {
    ...
    stringParam {
      name ('PRESEED_URL')
      defaultValue('${ARTF_WEB_URL}/genesis/preseed.cfg')
      description('URL of preseed.cfg file')
    }
    stringParam {
      name ('NETBOOT_URL')
